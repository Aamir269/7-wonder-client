...
function CreateReview() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios
        .post(`${API_URL}/api/wonder/${wonderId}/reviews`, body, {
          headers: {
      ...
      <form onSubmit={handleSubmit} className="formCreateReview">
        <div className="formCreateReview">
          <label className="createReviewField">
            ...
            <textarea
              value={content}
              className="inputCreateReviewConfiguration"
              onChange={(e) => setContent(e.target.value)}
            />
          </label>
          <button
            type="submit"
            className="createBtn border-green-600 hover:border-green-900 bg-green-600 hover:bg-green-900 text-slate-100 rounded"
          >
            <p>Create</p>
          </button>
        </div>
